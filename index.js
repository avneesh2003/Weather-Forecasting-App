// Api Key : 1e802905a72b46a48f645803231307
const DefaultPlace = "uk";
...
const GetCurrentLocation = () => {
    window.navigator.geolocation.getCurrentPosition(e => GetWeather("", e.coords.latitude, e.coords.longitude), GetWeather(DefaultPlace));
}
...
const GetWeather =  (city, latitude, longitude) =>{
    if(city){
        varurl = `http://api.weatherapi.com/v1/forecast.json?key=1e802905a72b46a48f645803231307&q=${city}&days=7&aqi=no&alerts=no`;
    } else {
        
        var url = `http://api.weatherapi.com/v1/forecast.json?key=1e802905a72b46a48f645803231307&q=${latitude},${longitude}&days=7&aqi=no&alerts=no`;
    }
    //console.log(url);
